...
Представим, что нам поступил заказ на мега крутой интернет магазин по продаже квадрокоптеров. А заказчик, допустим сам Иван Гуглов, очень суровый мужик и просит сделать максимально быстро и качественно.
...
Вариант очень простой. Не требует совершенно ни каких знаниях JavaScript. Он подходит если мы студент 1го курса и нам очень нужны деньги на электронные сигареты.
...
Jade очень гибок в отличие от underscore. При помощи Jade мы можем не держать все данные у себя а отдавать наши шаблоны *.jade на сервер где например python подставит данные из базы и сформирует нам html, который сам же все и отрисует на стороне клиента.